...
- **YicoreIconButton** - 图标按钮（支持轮廓样式、自定义颜色、禁用状态、边框控制）
...
- **YicoreAppBar** - 自定义应用栏（支持标题、返回按钮、操作按钮、居中标题、自定义标题组件）
- **YicoreSegmentedControl** - 分段控制器（支持小、中、大三种尺寸、边框选项、平滑动画）
...
- **YicoreCourseSelector** - 课程选择器（底部弹窗，支持课程列表展示和选择）
...
- **YicoreTimeRangePicker** - 时间范围选择器（底部弹窗，格式：HH:mm-HH:mm）
...
├── main.dart                   # 主入口和演示页面
├── pages/                      # 页面目录
│   ├── basic_components_page.dart      # 基础组件演示页面
│   ├── advanced_components_page.dart   # 进阶组件演示页面
│   └── premium_components_page.dart    # 高级组件演示页面
...
### v1.0.6
- 新增 YicoreIconButton 图标按钮组件
- 支持轮廓样式、自定义图标颜色、禁用状态、边框
- 更新 YicoreAppBar 使用新的图标按钮组件
- 所有对话框标题栏统一使用新的图标按钮组件

### v1.0.5
- 重构项目结构，将组件演示分类到三个独立页面（基础组件、进阶组件、高级组件）
- 新增 YicoreCourseSelector 课程选择器（底部弹窗）
- 新增 pages 目录
